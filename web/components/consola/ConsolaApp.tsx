...
// ── App de la consola (reutilizable: página propia y overlay del landing) ────

export function ConsolaApp({ onClose }: { onClose?: () => void }) {
    ...
    // Preferencia de tema (día/noche) persistida — confinada al CRM.
    useEffect(() => {
        setModoNoche(localStorage.getItem('crm-tema') === 'noche');
    }, []);
    useEffect(() => {
        localStorage.setItem('crm-tema', modoNoche ? 'noche' : 'dia');
    }, [modoNoche]);
    ...
    return (
        <ToastProvider>
            <div className={cn('flex min-h-screen bg-background', modoNoche && 'dark')}>
                    ...
                    <header className="flex h-14 shrink-0 items-center justify-between gap-3 border-b border-border bg-card px-4">
                        ...
                        <div className="flex items-center gap-2.5">
                            ...
                            <button
                                type="button"
                                role="switch"
                                aria-checked={modoNoche}
                                onClick={() => setModoNoche((v) => !v)}
                                aria-label={modoNoche ? 'Cambiar a vista de día' : 'Cambiar a vista nocturna'}
                                title={modoNoche ? 'Vista de día' : 'Vista nocturna'}
                                className={cn(
                                    'relative inline-flex h-7 w-[52px] shrink-0 items-center rounded-full border transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-futuro-accent focus-visible:ring-offset-1',
                                    modoNoche
                                        ? 'border-futuro-corp bg-futuro-corp'
                                        : 'border-border bg-muted'
                                )}
                            >
                                <Sun
                                    className={cn(
                                        'absolute left-1.5 size-3.5 transition-opacity duration-200',
                                        modoNoche ? 'opacity-40 text-white/70' : 'text-amber-500'
                                    )}
                                    aria-hidden="true"
                                />
                                <Moon
                                    className={cn(
                                        'absolute right-1.5 size-3.5 transition-opacity duration-200',
                                        modoNoche ? 'text-futuro-sky' : 'opacity-40 text-muted-foreground'
                                    )}
                                    aria-hidden="true"
                                />
                                <span
                                    className={cn(
                                        'pointer-events-none relative z-10 size-5 rounded-full bg-white shadow-md transition-transform duration-200',
                                        modoNoche ? 'translate-x-[26px]' : 'translate-x-0.5'
                                    )}
                                />
                            </button>
